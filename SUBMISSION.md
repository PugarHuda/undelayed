...
| Executor books | `--report`: won, lost, pending, fees actually earned | `src/bot.ts` |
| Executor economics | The fee is a prize in a race, and the losers pay to enter. Measured gas (356,375 to execute, 82,947 to request — our own transactions) at the live gas price and FTSO FLR/USD gives the margin per win, the cost per loss, the smallest payment that pays for itself, and **the win rate an executor needs to break even**. At today's prices that is 0.3%, and the binding constraint is not gas but the 0.1 XRP minimum system fee | `src/economics.ts` |
| Executor decisions, testable | Execute / wait / skip pulled out of the loop into a pure function, so the part that spends money has tests: exclusivity boundaries, missing ledger time, delayed-then-released, and the case where exclusivity and a rate limit both apply | `src/decide.ts` |
| Split invoicing | Grossing up for one payment and then splitting underpays the merchant by the extra pieces' fees. `invoiceSplit` grosses up against the pieces that will actually be sent, and the test asserts the naive route underpays | `src/fees.ts` |
| Dashboard | Raw getter vs real state, capacity, simulator, split planner, a merchant checkout that prices a USD basket off the FTSO and shows every deduction, and the executor's own margin at the live gas price | `dashboard/` |
...
| Render QA | Chromium + WebKit. Console errors, horizontal overflow, `100dvh` drift, **text painted over by something opaque**, **panels still showing their loading text**, and **a number that failed to compute** — `NaN` in a fee table is a wrong answer presented with total confidence, and no type checker sees it because `NaN` is a number. It found a masthead that pushed the page 19px wide on a phone and a hero canvas rendering through the opening paragraph — both by eye at the time, and the coverage check now catches that class automatically. The suite runs a deliberately broken fixture first and fails if it comes back clean | `qa/render.mjs`, `qa/fixtures/` |
