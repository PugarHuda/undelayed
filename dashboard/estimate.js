 ...
 */
import { predict, ubaToAmg } from "./limiter.js";
...
function phrase(seconds) {
    ...
    if (seconds <= 900n)
        return "within about fifteen minutes";
    if (seconds <= 3600n)
 ...
 */
export function estimate(snap, paidUba) {
 ...
 * agreeing to.
 */
export function checkoutLine(snap, paidUba) {
    const e = estimate(snap, paidUba);
    if (e.rejected)
        return e.text;
    const xrp = (u) => (Number(u) / 1e6).toLocaleString("en-US", { maximumFractionDigits: 6 });
    const why = e.reason === "none"
        ? ""
        : e.reason === "large"
