...
export const COSTON2_MEASURED = {
    executeGas: 356375n,
    requestGas: 82947n,
    gasPriceWei: 650000000000n,
    attestationFeeWei: 1000n,
};
const WEI = 1e18;
const UBA = 1e6;
const usdOf = (wei, nativeUsd) => (Number(wei) / WEI) * nativeUsd;
export function attempt(paymentUba, cfg, c) {
 ...
 */
export function payingPayment(cfg, c) {
    ...
    if (attempt(capped, cfg, c).winMarginUsd < 0)
        return null;
    ...
    for (let paid = cfg.minimumFeeUba; paid <= capped; paid += 1000n) {
        if (attempt(paid, cfg, c).winMarginUsd >= 0)
            return paid;
    }
    return capped;
}
 ...
 */
export function breakEvenWinRate(paymentUba, cfg, c) {
